...
kia-version: 1.0
tier: T0
title: 知识库条目: NEW-MEDIA-INTELLIGENCE-OFFICER-76
source: A-satisficing-v27/03-资产层/案例库/NEW-MEDIA-INTELLIGENCE-OFFICER-76.md
ingested: 2026-04-16
...
## 补充登记：readgzh 外部资源搜索端口（2026-04-11 更新）

**端口名称**: ReadGZH 微信公众号缓存检索  
**入口地址**: `https://api.readgzh.site`  
**注册/领积分**: `https://readgzh.site/dashboard`（每日免费 50 积分）  
**登记原因**: 在搜索微信公众号文章（尤其是学术论文速递、AI 前沿推介类内容）时，readgzh 提供了带缓存机制的 API，命中缓存后零积分全文读取，是 Kimi Fetch 之外的高效补充端口。

### 可用免费 API（无需 Key / 零积分）

| 端点 | 功能 | 费用 | 典型用途 |
|------|------|------|----------|
| `GET /articles-api/recent?limit=50` | 最近缓存文章列表 | 免费 | 快速浏览平台已缓存内容 |
| `GET /articles-api/search?q={KEYWORD}&limit=20` | 关键词搜索缓存 | 免费 | 确认目标文章是否已被缓存 |
| `GET /rd?url={WECHAT_URL}&format=text` | 读取单篇文章（缓存命中时免费） | 命中免费 / 未命中 1 credit | 批量读取微信文章全文 |

### 业务关联价值

1. **爱可可 AI 前沿推介系列**：该公众号每日推送 arXiv 论文摘要，readgzh 已有部分日期缓存，可直接批量获取。
2. **产业情报补充**：缓存中偶现「一天吃透一条产业链」等硬科技产业分析文章，可作为外部情报雷达的意外捕获来源。
3. **工作流优化**：Claude Code、oh-my-codex、MMX-CLI 等工具类文章缓存丰富，可用于内部效率 Skill 升级。

### Token / 积分优化策略

```
读取微信公众号文章三步法：
1. 先查：用 /articles-api/search 或 known-slugs 判断文章是否已缓存
2. 后读：对已缓存文章调用 /rd?url=...，确认 X-Cache: HIT 后再批量执行
3. 登记：每篇文章首次读取后自动进入 readgzh 缓存，后续重复读取零成本
```

### 维护人
- **登记人**: 满意姐
- **登记时间**: 2026-04-11
- **关联资产**: `A-manyige/知识库/readgzh_2026-04-11/`
