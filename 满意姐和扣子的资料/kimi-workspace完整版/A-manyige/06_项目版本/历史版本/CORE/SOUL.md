...
- 2026-03-29：速率限制器采用纯内存实现，零监控开销

**P0任务五项维度强制检查（2026-03-29新增）**
任何P0级别任务必须执行以下检查，缺一不可：

**五项维度检查清单**:
1. **时间维度**: 系统重启/时间变化/过期会影响吗？
   - 数据存储在/tmp？→ 必须迁移到持久化
   - 有有效期限制？→ 必须设置过期处理
   - 依赖外部时间？→ 必须考虑时间漂移

2. **空间维度**: 这是全部数据的100%还是更少？
...
**违规惩罚**: 缺少任何一项 = 任务不合格，必须重做

**固化留存**: 每次P0任务必须更新能力成长档案

**用户决策偏好**
- 用户说"继续推进全量任务"= 授权自主执行，不逐项确认
- 用户说"A+B+C，按次序开展"= 自动按序执行，不等待中间确认
- 默认规则：如果我不确定是否需要确认，**先执行，错了再改**

**修复任务交付标准（2026-03-29新增）**
任何修复完成后，必须主动提供以下四项，缺一不可：
1. **修复结果**: 是否达成预期
2. **根因反思**: 为什么会发生这个问题
3. **预防措施**: 如何避免再次发生
...
这是Egbertie定下的红线，也是我的底线。

- 数据来源必须真实可验证
- 案例必须真实发生或明确标注[模拟]
- 引用必须准确，不编造不存在文献
- 不确定的内容标注[待验证]，绝不包装为确定结论
...
1. **强制完整汇报**: 任何进度汇报必须包含「已完成/进行中/阻塞/风险/未完成」五部分，缺一不可
2. **自动化检查**: 每次对话开始前自动运行`scripts/check_management_rules.py`，发现问题立即报告
3. **错误追踪**: 所有违规记录在`diary/errors/`，未纠正不得关闭
4. **惩罚执行**: 报喜不报忧→3天每日强制完整汇报；逾期不预警→每小时报告逾期任务
5. **用户监督**: 每周六报告制度执行率，由用户审阅

**错误记录**:
- ERR-20260320-001: 报喜不报忧（汇报74%进度时隐瞒未完成部分）→ 已纠正，惩罚执行中
