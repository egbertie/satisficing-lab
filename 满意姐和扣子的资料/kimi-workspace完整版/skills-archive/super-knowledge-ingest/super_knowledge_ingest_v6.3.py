...
V6.3更新：
- 修复命名重复问题：使用路径哈希确保唯一性
- 格式: {filename}_{path_hash}_{type}_v6.json
- 避免同名文件覆盖
...
# Token消耗预估与效益红线（S3可观测输出）
TOKEN_COST_ESTIMATE = """
单次文件入库Token消耗估算：
- 小文件（<10KB）: ~200-500 tokens
- 中文件（10KB-100KB）: ~500-1500 tokens  
- 大文件（100KB-1MB）: ~1500-3000 tokens
- 超大文件（>1MB，截断处理）: ~3000-5000 tokens
- 平均: ~800 tokens/文件
"""

# Token效益红线 - 硬性约束
TOKEN_RED_LINES = {
    'max_per_file': 5000,       # 单个文件入库不得超过5K tokens
    'max_per_batch': 50000,     # 批量入库不得超过50K tokens
    'max_per_day': 200000,      # 单日不得超过200K tokens
...
# Token优化空间评估
TOKEN_OPTIMIZATION = {
    'caching_opportunity': '高 - 元数据缓存可节省40%',
    'batching_opportunity': '高 - 批量处理可节省30%',
    'compression_opportunity': '中 - 大文件压缩可节省20%',
...
'''
        ...
        try:
            ...
            # 验证描述
            assert "lines" in meta.description, f"Description should mention line count: {meta.description}"
