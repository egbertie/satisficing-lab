#!/usr/bin/env bash
# survey-designer - Design reference and helper tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${SURVEY_DESIGNER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/survey-designer}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
survey-designer v$VERSION

Design reference and helper tool

Usage: survey-designer <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "survey-designer v$VERSION" ;;
