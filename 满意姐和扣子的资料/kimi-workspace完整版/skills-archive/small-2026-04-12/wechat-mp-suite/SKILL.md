...
| 📥 爬虫 | URL → Markdown + 本地图片 | "下载这篇公众号文章" |
...
```
1. 搜索：node scripts/search/search_wechat.js "AI教程" -n 5 -c
2. 洗稿：使用 OpenClaw 自然语言指令（"帮我洗稿"）
3. 排版：node scripts/typeset/wechat-dual-copy.js article.md
...
```
1. 爬虫：python3 scripts/spider/main.py https://mp.weixin.qq.com/s/xxx ./output
2. 写作：使用 OpenClaw 写作指令（"用刘润风格改写"）
3. 排版：node scripts/typeset/wechat-dual-copy.js article.md
4. 远程发布：scripts/publisher/publish-remote.sh article.md
```
