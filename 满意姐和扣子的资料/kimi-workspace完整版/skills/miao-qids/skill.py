...
class IDSDetector:
	...
	def load_models(self):#加载模型
		try:
			...
			qnn_path=os.path.join(self.model_dir,"best_freeze_e18_a52.2.pkl")
			...
			if not os.path.exists(cnn_path):
				raise FileNotFoundError(f"CNN模型文件不存在:{cnn_path}")
			if not os.path.exists(qnn_path):
				raise FileNotFoundError(f"QNN模型文件不存在:{qnn_path}")
			...
			logger.info(f"模型加载成功:CNN→{self.cnn_model.latent_dim}维,QNN→{self.qnn_model.total_params}参数")
...
def main():#主函数
	...
	parser.add_argument('--host',default='127.0.0.1',help='服务IP(默认127.0.0.1)')
	parser.add_argument('--port',type=int,default=49160,help='服务端口(默认49160)')
	parser.add_argument('--model-dir',default='/media/sherryyann/field3/FW',help='模型文件路径')
